...
Nothing that runs by default may read `fixtures/corpus/`. That directory is
...
producer, use `fixtures/epub/long.epub` — eight chapters, four to five pages
each under the test metrics — instead of reaching for the corpus. It is
generated by `fixtures/epub/build-long.py` rather than checked in as source
like the fixtures `build.sh` handles, because sixty kilobytes of filler is
not worth reading in a diff. `long_enough_to_walk_out_of` in
chapbook-reader's session tests pins the shape the restore tests depend on,
so retuning the generator fails loudly instead of quietly making those
tests cover nothing.
